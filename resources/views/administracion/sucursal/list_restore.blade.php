...
{{--             <div class="col-2">
                <a href="{{ route('sucursal.create') }}" class="btn float-right colorCyan" role="button">+ Add sucursal</a>
            </div> --}}
                               ...
                               <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center">
                                                <button type="button" class="btn paddBto" id="btoRestore_{{$sucursal->id}}" onclick="activarSucursal({{$sucursal->id}})" >
...
@section('scripts')
    ...
    function activarSucursal(sucursalId){
        $.ajax({
                type: "POST",
                dataType: "json",
                url: '/sucursal/restore',
                data: {
                    sucursalId,
                    '_token':"{{ csrf_token() }}"
                    },
                success: function(data){
                    if(data.status===200){
                        $(`#btoRestore_${sucursalId}`).css('display','none');
                        toastr.success("Sucursal Activada");
                    }else{
                        $(`#btoRestore_${sucursalId}`).css('display','inline-block');
                        toastr.error("Sucursal No Activada");
                    }
                }
            });
    }
